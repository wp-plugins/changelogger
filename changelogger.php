			...
			if ( !is_wp_error( $api ) && current_user_can('update_plugins') ) {
				...
				if( isset($api->sections['changelog']) ) {
					...
					if( $section_exists ) {
						...
						$output .= sprintf(__('What has changed in version %1$s', 'changelogger'), trim( str_replace($search, $replace, $changelog_result[0][0]) ));
...
div.CLOS-message h4, div.CLOS-message p {
	margin: 0;
	display: inline;
}
